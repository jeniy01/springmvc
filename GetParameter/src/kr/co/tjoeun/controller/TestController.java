...
public class TestController {
	...
	@GetMapping("/test5/{number1}/{number2}/{number3}/{number4}")
	public String test5(@PathVariable int number1, @PathVariable int number2, @PathVariable int number3, @PathVariable int number4) {
	...
	//요청에서 넘어오지 않은 값(num3)을 받는 경우 400에러 발생함
	@GetMapping("/test9")
	...
	//요청에서 넘어오지 않은 값(num3, num4)을 받는 경우 required: false로 설정하면 지정된 이름의 파라미터가 없는 경우 null이 주입됨
	//@RequestParam(required=false) int num3
	//  ㄴ 일단 null값으로 받은 다음 int type으로 형변환 하려고 함 <-- 오류 발생
	//@RequestParam(required=false) String num4 <-- null을 주입 받음
	@GetMapping("/test10")
