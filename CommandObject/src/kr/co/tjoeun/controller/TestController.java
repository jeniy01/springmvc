...
public class TestController {
	...
	//이때, HttpServletRequest 객체에 자동으로 저장되는 이름은 클래스의 이름으로 됨(첫 글자가 소문자로 되어서 저장됨 --> number1 : ${requestScope.dataBean1.number1 })
	//(@ModelAttribute DataBean1 bean1) <-- parameter로 선언된 객체(DataBean1)를 HttpServletRequest 객체에 담아서 jsp로 전달한다는 의미
	@PostMapping("/test1")
